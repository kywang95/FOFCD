...
for (iloc in 1:length(locations)){
  ...
  for (irep in 1:repitition){
    ...
    #############################################################################
    time.gam <- Sys.time()
    res.gam <- try(fregre.gsam.vs(data=ldat, y="y", numbasis.opt=T
                                  , dcor.min = 0.01
    ...
    pred.gam.train <- predict(res.gam, ldat)
    print(feature.gam)
    time.gam
    # print(mean((matrix(pred.gam.train, ncol = 1) - Train_y)^2, na.rm = T))
    
    # predict
    pred.gam.test <- predict(res.gam, newldat)
    MSEP.gam <- mean((matrix(pred.gam.test, ncol = 1) - Test_y)^2, na.rm = T)
    ...
    res.fofcd.fb <- try(fregre.gsam.vs(data=ldat, y="y", numbasis.opt=T
                                       , dcor.min=1e-8, alpha = 1
    ...
    res.fofcd.osb <- try(fregre.gsam.vs(data=ldat, y="y", numbasis.opt=T
                                        , dcor.min=1e-8, alpha = 1
                                        , include = features.selected
                                        # , trace = T
    ))
    ...
    res.fLARS <- #try(
      fregre.gsam.vs(data=ldat, y="y", numbasis.opt=T
                     , dcor.min=0, alpha = 10
                     , include = features.selected
                     , AIC = FALSE 
                     # , trace = T
    ...
    res.SHFIM <- try(fregre.gsam.vs(data=ldat, y="y", numbasis.opt=T
                                    , dcor.min=1e-8, alpha = 1
                                    , include = features.selected
                                    # , trace = T
    ))
    ...
    result.MSEP[irep, ] <- c(MSEP.fofcd.fb, MSEP.fofcd.osb, MSEP.fLARS, MSEP.gam, MSEP.SHFIM)
    ...
    result.feature[8*3+(1:length(feature.gam)), irep] <- feature.gam
    result.feature[8*4+(1:length(feature.SHFIM)), irep] <- feature.SHFIM
  }
  colnames(result.MSEP) <- c("fofcd.fb", "fofcd.osb", "fLARS", "gam", "SHFIM")
  colnames(result.feature) <- paste0("rep_", 1:repitition)
  write.csv(result.MSEP, paste0("MSEP_of_", location, ".csv"))
